...
sfunction premium($token) {
	...
	CURLOPT_RETURNTRANSFER => true,
        ...
        CURLOPT_CUSTOMREQUEST => "POST",
        CURLOPT_POST => 1,
		CURLOPT_POSTFIELDS => "{"clientSdkMetadata":{"source":"client","integration":"custom","sessionId":"3ff52aaa-75cf-426b-93a0-8aef52f2b44b"},"query":"mutation TokenizeCreditCard($input: TokenizeCreditCardInput!) {   tokenizeCreditCard(input: $input) {     token     creditCard {       bin       brandCode       last4       binData {         prepaid         healthcare         debit         durbinRegulated         commercial         payroll         issuingBank         countryOfIssuance         productId       }     }   } }","variables":{"input":{"creditCard":{"number":"5154 6200 1875 2814","expirationMonth":"6","expirationYear":"2026","cvv":"118"},"options":{"validate":false}}},"operationName":"TokenizeCreditCard"}",
        CURLOPT_HTTPHEADER => array(
